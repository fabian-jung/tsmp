...
std::string decl_t::get_name(std::string prefix, namespace_option_t namespace_option) const
{
    ...
    return fmt::format("{}{}{}{}", prefix, namespace_str, namespace_str.empty() ? "" : "::", name);
}

pointer_t::pointer_t(const type_t* type, cv_qualifier_t cv_qualifier)
    : type(std::move(type))
...
constant_array_t::constant_array_t(const type_t* type, std::size_t size)
    : type(type)
    , size(size)
{
}
...
{
    return fmt::format("{}[{}]", type->get_name(prefix, namespace_option), size);
}
...
reference_t::reference_t(const type_t* type, cv_qualifier_t cv_qualifier, ref_qualifier_t ref_qualifier)
    : type(std::move(type))
    , cv_qualifier(cv_qualifier)
...
builtin_t::builtin_t(std::string name)
    : name(name)
{
}
...
record_t::record_t(std::string name, std::string qualified_namespace, bool is_struct)
    : decl_t(std::move(name), std::move(qualified_namespace))
    , is_struct(is_struct)
{
}
...
{
    auto template_parameter_list =
        template_arguments.empty()
            ? ""
            : fmt::format("<{}>", transform_join(template_arguments, ", ", [prefix](const template_argument_t& arg) {
                              return arg.get_value(prefix);
                          }));
    std::string namespace_str;
    if (parent != nullptr) {
        namespace_str = fmt::format("{}{}{}",
                                    namespace_str,
                                    namespace_str.empty() ? "" : "::",
                                    parent->get_name("", namespace_option_t::unqualified));
    } else {
    ...
    auto decl = fmt::format("{}{}{}{}{}",
                            ...
                            name);
    return fmt::format("{}{}", decl, template_parameter_list);
}
