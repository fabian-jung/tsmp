...
struct type_t
{
    ...
    virtual std::string get_name(std::string prefix = "",
                                 namespace_option_t namespace_option = data::namespace_option_t::unqualified) const = 0;
};
...
struct decl_t : public type_t
{
    ...
    std::string get_name(std::string prefix = "",
                         namespace_option_t namespace_option = data::namespace_option_t::unqualified) const override;
};
...
struct constant_array_t : public type_t
{
    ...
    constant_array_t(const type_t* type, std::size_t size);
...
struct record_t : public decl_t
{
    ...
    record_t(std::string name, std::string qualified_namespace, bool is_struct);
...
struct formatter<data::parameter_decl_t>
{
    ...
    auto format(const data::parameter_decl_t& param, FormatContext& ctx) const
    {
        return fmt::format_to(ctx.out(),
                              "{} {}{}",
                              param.type ? param.type->get_name() : "<unknown>",
                              param.name,
                              param.is_pack ? "..." : "");
    }
};
