...
public:
    ...
    template <class T>
    decltype(auto) allocate() const {
        return reinterpret_cast<T*>(::operator new(sizeof(T)));
    }

    template <class T>
    decltype(auto) emplace(const T* element) {
        auto [it, success] = fetch<T>().emplace(element);
        return *it;
    }
    ...
    template <class T>
    using entry_container_t = std::set<const T*, name_compare_t<T>>;
