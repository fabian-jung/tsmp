    ...
    auto format(namespace_wrapper_t const& arg, FormatContext& ctx) const
    {
        ...
        for (const auto& record : arg.records) {
            ...
            if (!record->template_arguments.empty()) {
                fmt::format_to(std::back_inserter(record_definition),
                               "    template <{0}>\n",
                               fmt::join(record->template_arguments, ", "));
                std::vector<std::string> names;
                std::transform(record->template_arguments.begin(),
                               record->template_arguments.end(),
...
std::string render_function_description(const std::vector<function_decl_t>& functions)
{
    ...
    for (const auto& f : functions) {
        if (f.name == "~")
            continue;
        result += fmt::format("\t\t\tfunction_description_t{{ {0}, \"{1}\", static_cast<{2} (value_type::*)({3}) "
                              "{4}{5}>(&value_type::{1}) }},\n",
                              i++,
                              f.name,
                              f.result ? f.result->get_name("typename GlobalNamespaceHelper::") : "<unknown>",
                              transform_join(f.parameter,
                                             ", ",
                                             [](const parameter_decl_t& p) {
                                                 return p.type ? p.type->get_name("typename GlobalNamespaceHelper::")
                                                               : "<unknown>";
                                             }),
                              f.is_const ? "const" : "",
                              f.ref_qualifier);
...
std::string render_proxy_functions(const std::vector<function_decl_t>& functions)
{
    ...
    for (const auto& function : functions) {
        ...
        for (auto& p : function.parameter) {
            if (auto ref = dynamic_cast<const data::reference_t*>(p.type);
                ref != nullptr && ref->ref_qualifier == data::ref_qualifier_t::lvalue) {
        ...
        if constexpr ((std::is_base_of_v<Base, value_type> || ...)) {{
            ...
            function.name,
            transform_join(function.parameter,
                           ", ",
                           [](const parameter_decl_t& param) {
                               return fmt::format("{} {}{}",
                                                  param.type ? param.type->get_name("typename GlobalNamespaceHelper::")
                                                             : "<unknown>",
                                                  param.name,
                                                  param.is_pack ? "..." : "");
                           }),
            ...
            function.is_virtual ? fmt::format("{}", function.result->get_name("typename GlobalNamespaceHelper::"))
                                : "decltype(auto)",
            param_list.empty() ? "" : ", ",
            fmt::join(param_list, ", "),
...
struct reflect<{0}> {{
    ...
    for (const auto& record : records) {
        ...
        result += fmt::format(reflect_trait_template,
        ...
        result += fmt::format(proxy_trait_template,
...
{}
    ...
    fmt::print("List of records to be rendered:\n{}\n",
               transform_join(aggregator.fetch<record_t>(), "\n", [](const auto& record) {
                   return record->get_name("typename GlobalNamespaceHelper::");
               }));
