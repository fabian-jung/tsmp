...
struct tsmp_argument_adjuster_t
{
    ...
    clang::tooling::CommandLineArguments operator()(clang::tooling::CommandLineArguments args, clang::StringRef file)
    {
        ...
        std::transform(
            builtinIncludePath.begin(), builtinIncludePath.end(), std::back_inserter(args), add_include_flag);
        return args;
    };

    static std::string add_include_flag(std::string path) { return "-I" + path; }
};
...
int main(int argc, const char* argv[])
{
    ...
    if (argc < 3) {
        fmt::print(std::cerr,
                   "Not enough parameters supplied. Usage is ./introspect_tool <input file> [<additional input files>] "
                   "<header>\n");
        return 1;
    }
    ...
    const auto compilation_database =
        clang::tooling::CompilationDatabase::loadFromDirectory(std::filesystem::current_path().native(), error);
    if (!compilation_database) {
        const auto current_path = std::filesystem::current_path().string();
        fmt::print(std::cerr,
                   "Compilation failed with error \"{}\". Maybe compilation database not found under {}\n",
                   error,
                   current_path);
    }
    ...
    if (!std::filesystem::exists(header_dir)) {
        fmt::print(std::cerr,
                   "Could not generate output. Directory {} requested for output does not exist.\n",
                   header_dir.string());
        return 1;
    }
